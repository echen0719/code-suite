...
def main(accountID, townID, townName):
	...
	requestPoint = '%20{}%20{}'.format(accountID, subID)
	url = 'https://api.edmundsgovtech.cloud/wipp-core/v1/wippUtils/{}'.format(requestPoint)

	# saving some space
	utilityOwnerName = propertyLocation = ownerAddress = None
	...
	for url in endpoints:
		...
		if 'ownerName' in data:
			...
			ownerAddress = convertToAddress(ownerStreet, ownerCitySt, ownerZip)
			landValue = data['landValue']
			improvementValue = data['imprValue']
			netValue = data['netValue']
			foundData = True

	return [ownerName, propertyLocation, ownerAddress, T2026, T2025, T2024, T2023, landValue, improvementValue, netValue]
